...
#include <renderer_d3d12/resource/d3d12_render_atom.h>
	...
	: _render_device(render_device)
{
...

	{
		...
		_placement_heap_allocator = new D3D12HeapAllocator(device, desc);
	}
	
	{
		D3D12ResourceAllocatorDesc desc;
		desc.size = 1024 * 1024 * 128;
		desc.slot_size = D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT;
		desc.flags = D3D12_HEAP_FLAG_NONE;
		desc.type = D3D12_HEAP_TYPE_UPLOAD;
		_dynamic_resource_allocator = new D3D12ResourceAllocator(device, desc);
	}
...
D3D12RenderResource* ResourceManager::lookup_resource(RenderHandle resource_handle)
{
	void* resource = nullptr;
	switch (resource_handle.type)
	{
	case Texture1D:
	case Texture2D:
	case Texture3D:
	case TextureCube:
		resource = _textures.get_resource(resource_handle.index);
		break;
	case RenderTarget:
		resource = _render_targets.get_resource(resource_handle.index);
		break;
	case ConstantBuffer:
	case VertexBuffer:
	case IndexBuffer:
		if (resource_handle.dynamic)
			resource = _dynamic_buffers.get_resource(resource_handle.index);
		else
			resource = _static_buffers.get_resource(resource_handle.index);
	...
	return reinterpret_cast<D3D12RenderResource*>(resource);
}
...
RenderHandle ResourceManager::create_texture(ID3D12GraphicsCommandList *command_list, const D3D12TextureDesc &desc, const void *data)
{
	...
	D3D12HeapAllocator* heap_allocator = _placement_heap_allocator;
	D3D12Allocation heap_allocation = heap_allocator->allocate(required_resource_size);
	ID3D12Heap* heap = heap_allocator->get_heap(heap_allocation);
	D3D12HeapOffset heap_offset = heap_allocator->get_offset(heap_allocation);
	...
	// Setup sub resource mapped_data
	...
	D3D12DescriptorAllocation srv_allocation = _cbv_srv_uav_descriptor_allocator->allocate(buffer_count);

	// create cpu descriptor handle for resource
	device->CreateShaderResourceView(resource.Get(), 
		&srv_desc, 
		_cbv_srv_uav_descriptor_allocator->get_cpu_handle(srv_allocation));
	
	D3D12Texture texture = {};
	texture.aligned_size = required_resource_size;
	texture.size = required_resource_size;
	texture.placed_resource.allocation = heap_allocation;
	texture.placed_resource.resource = resource;
	texture.srv_allocation = srv_allocation;
	texture.resource_state = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
	texture.type = Texture2D;


	return _textures.add_resource(texture);
}
...
RenderHandle ResourceManager::create_render_target(const D3D12RenderTargetDesc &desc, RenderResourceContext &rrc)
{
	...
	if (desc.depth_stencil) {
		...
		render_target.dsv_allocation = _dsv_descriptor_allocator->allocate(1);
		...
		device->CreateDepthStencilView(render_target.resource.Get(), 
			&dsv_desc, 
			_dsv_descriptor_allocator->get_cpu_handle(render_target.dsv_allocation));
	}
	else {
		...
		render_target.rtv_allocation = _rtv_descriptor_allocator->allocate(1);
		render_target.resource_state = D3D12_RESOURCE_STATE_RENDER_TARGET;
		render_target.type = DepthStencilTarget;
		...
		device->CreateRenderTargetView(render_target.resource.Get(), 
			&rtv_desc, 
			_rtv_descriptor_allocator->get_cpu_handle(render_target.rtv_allocation));
	}
	
	return _render_targets.add_resource(render_target);
}
...
RenderHandle ResourceManager::create_vertex_buffer(const D3D12BufferDesc &desc, const void *data, RenderResourceContext &rrc)
{
	ID3D12Device *device = _render_device.device();
	...
	D3D12Allocation heap_allocation = _placement_heap_allocator->allocate(required_resource_size);
	ID3D12Heap* heap = _placement_heap_allocator->get_heap(heap_allocation);
	D3D12HeapOffset heap_offset = _placement_heap_allocator->get_offset(heap_allocation);
	ComPtr<ID3D12Resource> resource;
	...
	return _static_buffers.add_resource(buffer);
}

RenderHandle ResourceManager::create_index_buffer(const D3D12BufferDesc &desc, const void *data, RenderResourceContext &rrc)
{
	ID3D12Device *device = _render_device.device();
...
RenderHandle ResourceManager::create_constant_buffer(const D3D12BufferDesc &desc, const void *data, RenderResourceContext &rrc)
{
	ID3D12Device *device = _render_device.device();
	...
	D3D12Allocation resource_allocation = _dynamic_resource_allocator->allocate(required_resource_size * buffer_count);
	void* mapped_data = _dynamic_resource_allocator->get_mapped_data(resource_allocation);
	D3D12DescriptorAllocation cbv_allocation = _cbv_srv_uav_descriptor_allocator->allocate(buffer_count);
	
	for (unsigned i = 0; i < buffer_count; ++i) {
		D3D12_CONSTANT_BUFFER_VIEW_DESC cbv_desc = {};
		cbv_desc.BufferLocation = _dynamic_resource_allocator->get_virtual_address(resource_allocation) + (required_resource_size * i);
	...
	return _dynamic_buffers.add_resource(buffer);
}
...
public:
	...
	Stream& operator<<(const T& val) {
		...
		unsigned offset = _data.size();
		_data.resize(_data.size() + size);
		memcpy(&_data[offset], &val, size);
	}
private:
	std::vector<char> _data;
};

size_t serialize_root_params(ResourceManager* resource_manager, const D3D12RootParameterDesc* root_parameters,
	const D3D12RootParameterValue* root_values,
	unsigned root_parameter_count,
	void **blob)
{
	size_t blob_size = 0;
	for (unsigned i = 0; i < root_parameter_count; ++i) {
		const D3D12RootParameterDesc& root_parameter = root_parameters[i];
		switch (root_parameter.type)
		{
		case D3D12RootParameter::ROOT_CONSTANT:
		case D3D12RootParameter::ROOT_MULTIPLE_CONSTANTS:
			blob_size += sizeof(D3D12RootParameter);
			blob_size += sizeof(D3D12RootParameterConstants);
			break;
		case D3D12RootParameter::ROOT_CBV:
		case D3D12RootParameter::ROOT_SRV:
		case D3D12RootParameter::ROOT_UAV:
			blob_size += sizeof(D3D12RootParameter);
			blob_size += sizeof(RenderHandle);
			blob_size += sizeof(D3D12_GPU_VIRTUAL_ADDRESS);

			break;
		case D3D12RootParameter::ROOT_DESCRIPTOR_TABLE:
		{
			blob_size += sizeof(D3D12RootParameter);
			blob_size += sizeof(D3D12RootParameterDescriptorTable);

			int j = i;
			while (root_parameters[j].index == root_parameter.index) {
				blob_size += sizeof(RenderHandle);
				blob_size += sizeof(D3D12_CPU_DESCRIPTOR_HANDLE);
				j++;
			}
			i = j - 1;
			break;
	...
	*blob = malloc(blob_size);
	...
	for (unsigned i = 0; i < root_parameter_count; ++i) {
		...
		unsigned size = sizeof(D3D12RootParameter);
		...
		case D3D12RootParameter::ROOT_MULTIPLE_CONSTANTS:
		{
			size += sizeof(D3D12RootParameterConstants);

			auto blob_root_parameter = (D3D12RootParameter*)p;
			...
			auto blob_root_parameter_constants = (D3D12RootParameterConstants*)(p + sizeof(D3D12RootParameter));
		...
		case D3D12RootParameter::ROOT_UAV:
		{
			...
			void* blob_gpu_virtual_address = (void*)(p + sizeof(D3D12RootParameter));
			void* blob_render_handle = (void*)(p + sizeof(D3D12RootParameter) + sizeof(D3D12_GPU_VIRTUAL_ADDRESS));

			D3D12RenderResource* resource = resource_manager->lookup_resource(root_value.render_resource);
		...
		case D3D12RootParameter::ROOT_DESCRIPTOR_TABLE:
		{
			size += sizeof(D3D12RootParameterDescriptorTable);
			...
			auto blob_root_parameter_descriptor_table = (D3D12RootParameterDescriptorTable*)(p + sizeof(D3D12RootParameter));
			auto blob_root_parameter_descriptor_entry = (D3D12_CPU_DESCRIPTOR_HANDLE*)(p + sizeof(D3D12RootParameter) + sizeof(D3D12RootParameterDescriptorTable));

			unsigned j = i;
			while (root_parameters[j].index == root_parameter.index) {
				blob_root_parameter->size += sizeof(D3D12_CPU_DESCRIPTOR_HANDLE);
				blob_root_parameter_descriptor_table->num_source_descriptors++;
				//D3D12RenderResource* render_resource = lookup_resource(root_values[j].render_resource);
...
//{
//	size_t required_size = sizeof(D3D12InstancedRenderAtom);
