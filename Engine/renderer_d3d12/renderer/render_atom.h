...
struct InstancedRenderAtom : public RenderAtom
{
...
template <typename T>
inline bool state_changed(T current, T &cached)
{
	bool changed = (current != cached);
	cached = current;
	return changed;
}
	...
	StateCache *state_cache)
{
	...
	unsigned srv_count = state_changed(atom->srv_hash, state_cache->srv_hash) ? atom->srv_count : 0;
	unsigned uav_count = state_changed(atom->uav_hash, state_cache->uav_hash) ? atom->uav_count : 0;
	unsigned cbv_count = state_changed(atom->cbv_hash, state_cache->cbv_hash) ? atom->cbv_count : 0;
	...
	unsigned dirty_sampler_count = state_changed(atom->sampler_hash, state_cache->sampler_hash) ? atom->sampler_count : 0;
